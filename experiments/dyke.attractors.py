...
if __name__ == '__main__':
    ...
    for each_K in range(1, RANGE_K, 1):
        K_list = []
        K_list.append(each_K)
        ...
        K_RESULT.append((K_list, omega,mu,five,seven,ten,st,nw))

        #print((K_list, omega,mu,five,seven,ten,st,nw))
        print(K_list,RANGE_K)

    t = time.localtime()
    current_time = time.strftime("%H:%M:%S", t)
    print(current_time)

    data_directory = str(os.getcwd())+"/data_attractors/" + str(time.time()) + "." + str(random.randint(100, 999)) + "." + exp_name
