...
def rates_of_change_system_state(system_state):
    ...
    # Local Species have two effects - one for Global E and one for Local E

    for s_i in range(SPECIES_K):
        biotic_force_FG += (system_state[s_i] * omega[0][s_i]) # Every species affects Global E

    for s_i in range(SPECIES_K):
        if s_i in local_population_index:
            biotic_force_FL += (system_state[s_i] * omega[1][s_i]) # Only Local species affect Local E and Local E can only affect local species

    rate_of_change[SPECIES_K+0] = (biotic_force_FG) # Global Force
    rate_of_change[SPECIES_K+1] = (biotic_force_FL) # Local Force
    #dE/dt = E* + F
